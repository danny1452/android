...
// data source class - extend page keyed data source
public class MovieDataSource extends PageKeyedDataSource<Long,Movie> {
    ...
    public void loadInitial(@NonNull LoadInitialParams<Long> params, @NonNull final LoadInitialCallback<Long, Movie> callback) {
        ...
        // use new method - use retrofit Call for queueing
        Call<MovieDBResponse> call = movieDataService.getPopularMoviesWithPaging(application.getApplicationContext().getString(R.string.api_key),1);// 1 first page

        call.enqueue(new Callback<MovieDBResponse>() {
            // override on response
            @Override
    ...
    public void loadAfter(@NonNull final LoadParams<Long> params, @NonNull final LoadCallback<Long, Movie> callback) {
        ...
        movieDataService = RetrofitInstance.getService();
        Call<MovieDBResponse> call = movieDataService.getPopularMoviesWithPaging(application.getApplicationContext().getString(R.string.api_key),params.key); // other pages
